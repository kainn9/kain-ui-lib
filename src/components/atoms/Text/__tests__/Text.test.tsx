...
test("text is correct color from color prop", () => {
  ...
  const textElement = screen.getByTestId("kain-ui-text")
...
test("text is correct font size from optional size prop", () => {
  render(<DefaultExample font="roboto" size="bingBong" />)
  const textElement = screen.getByTestId("kain-ui-text")
  const style = window.getComputedStyle(textElement)

  expect(style.fontSize).toEqual(`${sizes.bingBong}px`)
})
